...
const UCAT_SECTIONS = [
  { href: "/practice/vr",  icon: <BookOpen className="size-4" />,    label: "Verbal Reasoning",       short: "VR",  color: "text-blue-400"   },
  { href: "/practice/dm",  icon: <Brain className="size-4" />,        label: "Decision Making",        short: "DM",  color: "text-purple-400" },
  { href: "/practice/qr",  icon: <Calculator className="size-4" />,   label: "Quantitative Reasoning", short: "QR",  color: "text-amber-400"  },
  { href: "/practice/ar",  icon: <Eye className="size-4" />,           label: "Abstract Reasoning",     short: "AR",  color: "text-rose-400"   },
  { href: "/practice/sjt", icon: <Users className="size-4" />,         label: "Situational Judgement",  short: "SJT", color: "text-teal-400"   },
];
          ...
          {UCAT_SECTIONS.map((s) => (
            collapsed ? (
              ...
              <Link key={s.href} href={s.href} className={cn(
                ...
                isActive(s.href) ? "bg-teal-500 text-white" : "hover:bg-accent hover:text-accent-foreground"
              )}>
                <span className={isActive(s.href) ? "text-white" : s.color}>{s.icon}</span>
                <span className="flex-1">{s.label}</span>
                <span className={cn("text-xs font-bold", isActive(s.href) ? "text-white/80" : s.color)}>{s.short}</span>
              </Link>
            )
          ))}
