...
export function QuizQuestion({ question, selectedOptionId, correctOptionId, onSelectOption, isReviewMode = false }: QuestionProps) {
  ...
  useEffect(() => { setSelectedOption(selectedOptionId); setAnswered(!!selectedOptionId); }, [selectedOptionId]);

  const handleOptionSelect = (optionId: string) => {
    if (answered || isReviewMode) return;
    setSelectedOption(optionId); setAnswered(true); onSelectOption(question.id, optionId);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3 mt-6">
        {question.options.map((option, index) => {
          ...
          return (
            <div key={option.id} style={{ opacity: 1 }}>
