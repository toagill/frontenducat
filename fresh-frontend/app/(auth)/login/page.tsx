...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.data.emailVerified) { setNeedsVerify(true); setLoading(false); return; }
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#0B1F3A] p-4">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-8 text-center">
        ...
        <p className="text-gray-500 mb-6">
          We sent a verification link to <strong>{form.email}</strong>. Please check your inbox and click the link to activate your account.
        </p>
        {resent ? (
        ...
        <button onClick={() => setNeedsVerify(false)} className="text-sm text-gray-400 hover:text-gray-600">
          ← Back to login
        </button>
      </div>
    </div>
      ...
      <div className="hidden lg:flex flex-col justify-between w-1/2 p-12 text-white">
        ...
        <div>
          <h1 className="text-4xl font-bold leading-tight mb-4">
            Welcome back.<br />Let's keep practising.
          </h1>
        ...
        <div className="w-full max-w-md">
          <div className="bg-white rounded-2xl shadow-2xl p-8">
            ...
            <p className="text-gray-500 text-sm mb-6">
              ...
              <Link href="/register" className="text-teal-600 font-semibold hover:underline">Start free trial</Link>
            </p>
